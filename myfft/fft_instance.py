...
def get_fft_instance(fft_type, shape, **kwargs):
    assert fft_type in ["numpy", "scipy", "pyfftw"]
    if fft_type == "numpy":
        return get_numpy_fft(shape=shape, kwargs=kwargs) 
    elif fft_type == "scipy":
        return get_scipy_fft(shape=shape, kwargs=kwargs)
    elif fft_type == "pyfftw":
        return get_pyfftw_fft(shape=shape, kwargs=kwargs)
    else:
        raise NotImplementedError()

def get_numpy_fft(shape, fftn_shape=None, **kwargs):
...
def get_scipy_fft(shape, fftn_shape=None, **kwargs):
    import numpy.fft as numpy_fft
...
def get_pyfftw_fft(shape, fftn_shape=None, n_threads=1, **kwargs):
    try:
